//! FMI2 cross-validation against rumoca's built-in simulator on MSL models.
//!
//! Tests a curated set of MSL models that compile and simulate successfully
//! through the FMI2 C template pipeline. For each model we:
//! 1. Compile the model from the MSL source root
//! 2. Run rumoca's built-in diffsol simulator to get a reference trace
//! 3. Render FMI2 C source, compile with `cc`, run to get CSV trace
//! 4. Compare the two traces and assert they agree within tolerance
//!
//! Run with:
//! ```text
//! cargo test --release --package rumoca-test-msl --test fmi2_msl_test -- --ignored --nocapture
//! ```
//!
//! Environment variables:
//! - `RUMOCA_FMI2_MSL_MATCH=pattern` — filter models by substring match
//! - `RUMOCA_FMI2_MSL_LIMIT=N` — cap number of models tested
//! - `RUMOCA_FMI2_MSL_DT=0.0001` — forward Euler step size (default 0.0001)
//! - `RUMOCA_FMI2_MSL_TOLERANCE=0.20` — max allowed trace deviation (default 0.20)
...
fn check_release_mode() {
    ...
    {
        panic!(
            "\n\nERROR: FMI2 MSL tests must be run in RELEASE mode!\n\
             cargo test --release --package rumoca-test-msl --test fmi2_msl_test -- --ignored --nocapture\n"
        );
    }
}
...
/// Load FMI2 MSL target models from the discovered target list.
/// Generated by `fmu_target_discovery` test.
fn fmi2_target_models() -> Vec<String> {
    let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/msl_tests/fmi2_msl_targets.json");
...
fn apply_env_filters(names: &mut Vec<String>) {
    if let Ok(pattern) = std::env::var("RUMOCA_FMI2_MSL_MATCH") {
        let pattern = pattern.trim().to_string();
        if !pattern.is_empty() {
            names.retain(|n| n.contains(&pattern));
            println!("RUMOCA_FMI2_MSL_MATCH={pattern} → {} models", names.len());
        }
    }
    if let Ok(raw) = std::env::var("RUMOCA_FMI2_MSL_LIMIT")
        && let Ok(limit) = raw.trim().parse::<usize>()
        && names.len() > limit
    {
        names.truncate(limit);
        println!("RUMOCA_FMI2_MSL_LIMIT={limit} → {} models", names.len());
    }
}

fn fmi2_dt() -> f64 {
    std::env::var("RUMOCA_FMI2_MSL_DT")
        .ok()
        .and_then(|s| s.trim().parse().ok())
        .unwrap_or(0.0001)
}

fn fmi2_tolerance() -> f64 {
    std::env::var("RUMOCA_FMI2_MSL_TOLERANCE")
        .ok()
        .and_then(|s| s.trim().parse().ok())
        .unwrap_or(0.20)
}
...
/// Render FMI2 C model + test driver, compile with cc, run, return CSV output.
fn fmi2_simulate(
...
/// Compute max relative error between two traces, sampling at FMI2 time points.
...
enum ModelOutcome {
    ...
    Fmi2RenderFail(String),
    Fmi2CompileOrRunFail(String),
...
impl std::fmt::Display for ModelOutcome {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ModelOutcome::Fmi2RenderFail(e) => write!(f, "fmi2_render_fail: {}", truncate(e, 120)),
            ModelOutcome::Fmi2CompileOrRunFail(e) => {
                write!(f, "fmi2_c_fail: {}", truncate(e, 200))
            }
...
fn run_single_model(
    ...
    // Skip models with no states — FMI2 forward Euler has nothing to integrate
    ...
    let t_end = result
        ...
        .min(10.0); // Cap FMI2 forward Euler simulation time
    ...
    // 3. Run FMI2 C pipeline
    let csv = match fmi2_simulate(dae, model_name, t_end, dt) {
        Ok(csv) => csv,
        Err(e) => {
            if e.contains("render") {
                return ModelOutcome::Fmi2RenderFail(e);
            }
            return ModelOutcome::Fmi2CompileOrRunFail(e);
        }
    };
...
#[test]
#[ignore]
fn test_fmi2_vs_rumoca_msl() {
    check_release_mode();

    let dt = fmi2_dt();
    let tolerance = fmi2_tolerance();
    println!("FMI2 vs rumoca MSL cross-validation");
    ...
    let mut targets = fmi2_target_models();
    ...
    for (i, model_name) in targets.iter().enumerate() {
        ...
        let tag = match &outcome {
            ...
            ModelOutcome::Fmi2RenderFail(_) | ModelOutcome::Fmi2CompileOrRunFail(_) => {
                fmi2_fail += 1;
                "FMI2_FAIL"
            }
            ModelOutcome::TraceDeviation {
    ...
    // Individual model failures are expected (many MSL models use features the
    // FMI2 template doesn't yet handle). The test ensures no regressions in the
    // models that do work.
    assert!(
        tested > 0,
        "no models were tested end-to-end — FMI2 pipeline may be broken"
    );
