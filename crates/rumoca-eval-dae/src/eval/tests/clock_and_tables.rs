...
#[test]
fn test_eval_super_sample_value_form_preserves_clocked_var_value() {
    let mut env = VarEnv::<f64>::new();
    env.set("time", 0.0);
    env.set("u", 0.0);
    ...
    assert_eq!(
        value, 0.0,
        "value-form superSample(u, factor) must not lower to the clock tick for u"
    );
}
...
fn test_eval_builtin_sample_with_clock_returns_sampled_value_not_event_tick() {
    ...
    let sample_expr = rumoca_core::Expression::BuiltinCall {
        function: rumoca_core::BuiltinFunction::Sample,
...
fn test_eval_builtin_sample_start_interval_keeps_event_boolean_semantics() {
    ...
    let mut env = VarEnv::<f64>::new();
    let sample_event_expr = rumoca_core::Expression::BuiltinCall {
        function: rumoca_core::BuiltinFunction::Sample,
    ...
    let write_real_matrix = fn_call(
        ...
        vec![
            rumoca_core::Expression::Literal {
                value: rumoca_core::Literal::String("test.mat".to_string()),
                span: rumoca_core::Span::DUMMY,
            },
            rumoca_core::Expression::Literal {
                value: rumoca_core::Literal::String("A".to_string()),
                span: rumoca_core::Span::DUMMY,
            },
            arr(
                vec![
...
#[test]
// SPEC_0021: Exception - table-driven regression covering related state accessor intrinsics.
            ...
            &fn_call(
                ...
                vec![rumoca_core::Expression::FieldAccess {
                    base: Box::new(rumoca_core::Expression::VarRef {
                        name: rumoca_core::Reference::new("media"),
                        subscripts: vec![rumoca_core::Subscript::generated_index(
                            2,
                            rumoca_core::Span::DUMMY
                        )],
...
fn test_eval_builtin_sample_behaviour() {
    ...
    let sampled_value = rumoca_core::Expression::BuiltinCall {
        function: rumoca_core::BuiltinFunction::Sample,
        args: vec![var("u")],
        span: rumoca_core::Span::DUMMY,
    };
...
fn test_eval_builtin_cat_array_values() {
    let env = VarEnv::<f64>::new();
    let cat_expr = rumoca_core::Expression::BuiltinCall {
...
#[test]
fn test_table1d_constructor_loads_modelica_text_table_file() {
    let mut env = VarEnv::<f64>::new();
...
fn test_table1d_constructor_accepts_flattened_field_access_matrix() {
    ...
    env.dims = Arc::new(IndexMap::from([(
        "cellData.OCV_SOC_internal".to_string(),
        vec![2, 2],
    )]));
    set_array_entries(
        &mut env,
        "cellData.OCV_SOC_internal",
        &[2, 2],
        &[0.0, 0.5, 1.0, 1.0],
    );

    let table_expr = rumoca_core::Expression::FieldAccess {
        base: Box::new(var("cellData")),
...
#[test]
fn test_time_table_constructor_expands_matrix_rows_with_vector_columns() {
    let mut env = VarEnv::<f64>::new();
    ...
    assert!((next - 1.0).abs() < 1e-12, "next={next}");
    assert!((high - 1.0).abs() < 1e-12, "high={high}");
    assert!((low - 0.0).abs() < 1e-12, "low={low}");
}

#[test]
fn test_time_table_boolean_toggle_duplicate_first_knot_uses_right_limit_value() {
    let mut env = VarEnv::<f64>::new();
    ...
    assert!((before - 0.0).abs() < 1e-12, "before={before}");
    assert!((after - 1.0).abs() < 1e-12, "after={after}");
}

#[test]
fn test_time_table_constructor_prefers_richer_start_matrix_over_truncated_slots() {
    let mut env = VarEnv::<f64>::new();
    set_vector_var(&mut env, "table", &[0.05, 0.15]);
    env.set("n", 2.0);
    env.set("startValue", 0.0);
    ...
    let table_matrix = rumoca_core::Expression::If {
        branches: vec![(
            ...
            rumoca_core::Expression::If {
                branches: vec![(var("startValue"), true_start_matrix)],
                else_branch: Box::new(false_start_matrix),
                span: rumoca_core::Span::DUMMY,
            },
        )],
        else_branch: Box::new(arr(vec![arr(vec![lit(0.0), lit(0.0)], true)], true)),
        span: rumoca_core::Span::DUMMY,
    };

    env.dims = Arc::new(IndexMap::from([
        ("table".to_string(), vec![2]),
        ("table_dyn".to_string(), vec![1, 2]),
    ]));
    env.start_exprs = Arc::new(IndexMap::from([("table_dyn".to_string(), table_matrix)]));
